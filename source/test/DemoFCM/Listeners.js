...
AsyncStorage.getItem('lastNotification').then(data => {
    if (data) {
        console.log("AsyncStorage.getItem('lastNotification')");
...
AsyncStorage.getItem('lastMessage').then(data => {
    if (data) {
        // if notification arrives when app is killed, it should still be logged here
        console.log("AsyncStorage.getItem('lastMessage')");
...
export function registerAppListener(navigation) {
    FCM.on(FCMEvent.Notification, notif => {
        alert('TÔI VỪA NHẬ ĐƯỢC THÔNG BÁO', JSON.stringify(notif));
        ...
        if (notif.opened_from_tray) {
            ...
            // }
            setTimeout(() => {
                alert(`Nguyễn Ngọc Duy vừa click vào thông báo\n${JSON.stringify(notif)}`)
            }, 500)
        }
    ...
    setTimeout(function () {
        FCM.isDirectChannelEstablished().then(result => console.log('isDirectChannelEstablished', result));
    }, 1000);
}
