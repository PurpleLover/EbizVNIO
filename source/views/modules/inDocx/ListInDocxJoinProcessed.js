...
class ListInDocxJoinProcessed extends BaseComponent {
    constructor(props) {
        super(props);
        this.state = {
            //filter
    ...
    fetchData = async () => {
        ...
        const response = await fetch(
            `${constant.DEFAULT_API_URL}/api/vanbanden/vanbanthamgiaxuly/${this.props.userId}/${constant.DEFAULT_PAGE_SIZE}/${this.state.pageIndex}?query=`
        );
    ...
    onFilter() {
        if (!_.isNull(this.state.filterValue) && !_.isEmpty(this.state.filterValue)) {
            this.props.setFilterInfo(this.state.filterValue, constant.IN_DOCX_JOIN_PROCESSED);
...
export default connect(mapStateToProps, mapDispatchToProps)(ListInDocxJoinProcessed);